...
const { join, dirname, resolve, extname, relative } = require('path');
 ...
 *
 * @param {string} filePath File path to normalise.
...
/**
 * Get path relative to process cwd with normalised separators.
 *
...
function getRelativePath(filePath) {
	...
	return normalizePath('' === relPath ? absolutePath : relPath);
}
...
if (processSpecificFiles) {
	...
	specificFiles.forEach(file => {
		...
		if (stat.isDirectory()) {
			console.log(`  ✓ Scanning directory: ${file}`);
			const cssFromDir = findFiles(resolvedPath, '.css', []);
			const jsFromDir = findFiles(resolvedPath, '.js', []);

			if (cssFromDir.length === 0 && jsFromDir.length === 0) {
				console.log(`  ✗ No CSS/JS files found in directory: ${file}`);
			}

			cssFromDir.forEach((cssFile) => {
		...
		if (ext === '.css') {
			if (!allCssFiles.includes(relativePath)) {
				allCssFiles.push(relativePath);
			}
			console.log(`  ✓ Found CSS: ${relativePath}`);
		} else if (ext === '.js') {
			if (!allJsFiles.includes(relativePath)) {
				allJsFiles.push(relativePath);
			}
			console.log(`  ✓ Found JS: ${relativePath}`);
		} else {
